...
function sendNextQuestion(raceCode, playerId) {
  ...
  if (player.waitingDecision) return;

  const diff = player.inDirt ? 1 : (player.position < 300 ? 1 : player.position < 600 ? 2 : 3);
  const q = player.inDirt ? generateDirtQuestion() : generateQuestion(diff);
